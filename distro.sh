...
zip -j "$BUILT_PRODUCTS_DIR/$PROJECT-$TAGGED.zip" \
       "$BUILT_PRODUCTS_DIR/WincentIconUtility.pkg"
...
while read mode type sha1 path
do
  SUBMODULE_NOTES=$(cd $path && "$BUILDTOOLS_DIR/ReleaseNotes.sh" --tag-prefix="wincent-icon-util-")
...
while read mode type sha1 path
do
  SUBMODULE_NOTES=$(cd $path && "$BUILDTOOLS_DIR/ReleaseNotes.sh" --long --tag-prefix="wincent-icon-util-")
  if [ $(echo "$SUBMODULE_NOTES" | wc -l) -ne 1 ]; then
    echo -n "$path: " >> $NOTES
    echo "$SUBMODULE_NOTES" >> $NOTES
  fi
